 ...
 */
@HK2
public class StartContainerNGTest {
    ...
    @Test
    public void givenContainerIdRequestShouldStartContainer() {
        client.request(createResponse.getId(), sut);
    }

    @Test
    public void givenContainerIdAndHostConfigRequestShouldStartContainer() {
        HostConfig config = HostConfig.newBuilder().build();
        config.getPortBindings()
                .put("22/tcp",
                        asList(PortBinding.newBuilder()
                                .setHostPort(11022)
                                .build())
                );
        client.request(createResponse.getId(), config, sut);
    }
